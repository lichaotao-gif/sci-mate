...
function ResearchWorkspace() {
  ...
  const overviewTodos:Array<{id:string;title:string;detail:string;actionLabel:string;tone:string;icon:typeof ListTodo;action:()=>void}>=[];
  if(!projectCompleted){
    if(primaryTask)overviewTodos.push({id:"task",title:primaryTask.title,detail:`需要推进并确认「${primaryTask.output}」`,actionLabel:"打开任务",tone:"violet",icon:ListTodo,action:()=>{setFocusedTaskId(primaryTask.id);go("tasks")}});
    const unreadPaper=papers.find(paper=>paper.saved&&!paper.read);
    if(unreadPaper)overviewTodos.push({id:"paper",title:"核阅待确认的核心文献",detail:`还有 ${papers.filter(paper=>paper.saved&&!paper.read).length} 篇文献等待核阅，先从「${unreadPaper.title}」开始`,actionLabel:"开始核阅",tone:"cyan",icon:BookOpen,action:()=>openPaper(unreadPaper.id)});
    if(!confirmed)overviewTodos.push({id:"evidence",title:"确认候选研究路线",detail:"核查过程验证路线的证据、适用边界和待验证假设",actionLabel:"前往确认",tone:"amber",icon:ShieldCheck,action:()=>go("evidence")});
  }
  ...
  return <>
    ...
    {page==="overview"&&isDemo&&<div className="overview-dashboard">
      ...
      <section className="panel overview-todos">
        <header><div><span className="overview-todo-icon"><ListTodo size={18}/></span><span><strong>我的待办</strong><small>需要你处理或确认的研究事项</small></span></div>{overviewTodos.length>0&&<em>{overviewTodos.length} 项待处理</em>}</header>
        {overviewTodos.length?<div className="overview-todo-list">{overviewTodos.map((item,index)=>{const TodoIcon=item.icon;return <article key={item.id} className={`overview-todo-item ${item.tone}`}><span className="todo-order">0{index+1}</span><span className="todo-item-icon"><TodoIcon size={17}/></span><div><strong>{item.title}</strong><p>{item.detail}</p></div><Button variant={index===0?"default":"outline"} onClick={item.action}>{item.actionLabel}<ArrowRight size={14}/></Button></article>})}</div>:<div className="overview-todo-empty"><span><Check size={21}/></span><div><strong>当前没有待办事项</strong><p>你需要处理的研究任务已经完成，Agent 会继续监测新文献和研究变化。</p></div><Button variant="outline" onClick={()=>go("radar")}>查看科研雷达</Button></div>}
      </section>
