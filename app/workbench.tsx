...
export function PaperLibrary(p:Props){
 ...
 return <>
  ...
  {p.selected.length>0&&<div className="selection-bar"><span>已选择 <strong>{p.selected.length}</strong> 篇</span><button onClick={()=>p.setSelected([])}>取消选择</button><Button disabled={p.selected.length<2} onClick={()=>p.go("evidence")}><Network/>比较文献<ArrowRight/></Button></div>}
 </>;
}
...
export function Evidence(p:Props){
 ...
 return <>
  <div className="knowledge-hero"><div><span className="eyebrow">PROJECT KNOWLEDGE</span><h2>{p.goal}</h2><p>直接查看当前项目已经形成的知识结构；点击节点可在图谱内查看知识摘要与证据数量。</p></div><div className="knowledge-health"><strong>12</strong><span>知识节点<small>持续自动更新</small></span></div></div>
  <div className="knowledge-evidence-strip">{summaries.map(item=><button className={item.tone} key={item.label} onClick={()=>setNode(item.node)}><span>{item.label}</span><strong>{item.count}</strong><small>查看相关证据<ChevronRight size={13}/></small></button>)}</div>
  <KnowledgeGraph project={p.project} node={node} setNode={setNode} paperCount={p.papers.filter(x=>x.saved).length}/>
...
export function Tasks(p:Props){
 ...
 const startAgent=(x:Task)=>{setRunStates(prev=>({...prev,[x.id]:"running"}));setTimeout(()=>{setRunStates(prev=>({...prev,[x.id]:"review"}));toast.success("Agent 执行完成，请审核产出")},1100)};
...
Figure 1. 当前研究证据结构示意图（待生成）

Table 1. 推理方法、反馈来源与实验条件对比（待补充）
...
${p.papers.filter(x=>x.saved).map((x,i)=>`[${i+1}] ${x.title}. ${x.venue}, ${x.year}. ${x.url}`).join("\n")}`;
 ...
 function exportMarkdown(){downloadBlob(p.report,"text/markdown;charset=utf-8","SCIMate-阶段研究报告.md")}
 function exportWord(){const escaped=p.report.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");const html=`<!doctype html><html><head><meta charset="utf-8"><style>body{font-family:Arial,'Microsoft YaHei',sans-serif;margin:48px;color:#263b57;line-height:1.8}pre{white-space:pre-wrap;font:inherit}</style></head><body><pre>${escaped}</pre></body></html>`;downloadBlob(html,"application/msword;charset=utf-8","SCIMate-阶段研究报告.doc")}
 function exportPdf(){const win=window.open("","_blank");if(!win){toast.error("浏览器阻止了打印窗口，请允许弹窗后重试");return}win.opener=null;const escaped=p.report.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>SCIMate 阶段研究报告</title><style>@page{margin:20mm}body{font-family:Arial,'Microsoft YaHei',sans-serif;color:#263b57;line-height:1.8}pre{white-space:pre-wrap;font:inherit}</style></head><body><pre>${escaped}</pre><script>window.onload=()=>window.print()<\/script></body></html>`);win.document.close()}
 const preview=p.report.split("\n").map((line,i)=>line.startsWith("# ")?<h1 key={i}>{line.slice(2)}</h1>:line.startsWith("## ")?<h2 key={i}>{line.slice(3)}</h2>:line.startsWith("### ")?<h3 key={i}>{line.slice(4)}</h3>:line.startsWith("- ")?<p className="report-bullet" key={i}>{line}</p>:line.startsWith("  ")?<p className="report-indent" key={i}>{line.trim()}</p>:line?<p key={i}>{line}</p>:<div className="report-space" key={i}/>);
 return <>
  ...
  {p.researchArtifacts.length>0&&<section className="artifact-gallery"><div className="section-heading standalone"><h2>成果记录 <span className="sub-count">{p.researchArtifacts.length}</span></h2><span className="mini-meta">图、文、视频关联到同一研究项目</span></div><div className="artifact-grid">{p.researchArtifacts.map(item=><article className="panel artifact-card" key={item.id}><div className={`artifact-preview ${item.type}`}>{item.type==="report"?<><FileText size={34}/><span>MANUSCRIPT</span></>:item.type==="chart"?<><div className="mini-bars"><i/><i/><i/><i/></div><span>FIGURE 1</span></>:item.type==="image"?<><Network size={35}/><span>GRAPHICAL ABSTRACT</span></>:<><span className="video-play"><Play size={21}/></span><em>02:18</em><span>SUPPLEMENTARY VIDEO S1</span></>}</div><div className="artifact-card-body"><span className="tag">{item.type==="report"?"主文档":item.type==="chart"?"Figure / Table":item.type==="image"?"图形摘要":"补充视频"}</span><h3>{item.title}</h3><p>{item.createdAt}</p><button onClick={()=>toast.info(item.type==="report"?"请在下方在线预览论文稿":"成果已生成，可进入详情查看或导出")}>在线预览<ArrowUpRight size={14}/></button></div></article>)}</div></section>}
  {p.report?<div className="report-center"><section className="panel report-document"><div className="report-toolbar"><Tabs value={view} onValueChange={v=>setView(v as "preview"|"edit")}><TabsList className="view-tabs"><TabsTrigger value="preview">在线预览</TabsTrigger><TabsTrigger value="edit">编辑内容</TabsTrigger></TabsList></Tabs><div className="report-export"><span>导出报告</span><Button variant="outline" onClick={exportWord}><Download/>Word</Button><Button variant="outline" onClick={exportPdf}><Download/>PDF</Button><Button variant="outline" onClick={exportMarkdown}><Download/>Markdown</Button></div></div>{view==="preview"?<article className="report-online-preview">{preview}</article>:<textarea aria-label="编辑阶段研究报告" value={p.report} onChange={e=>p.setReport(e.target.value)}/>}</section><aside className="panel report-history"><div className="section-heading"><h2>生成记录</h2><span className="tag">{p.reportHistory.length} 个版本</span></div><div className="report-history-list">{p.reportHistory.map((item,index)=><button key={item.id} className={p.report===item.content?"active":""} onClick={()=>{p.setReport(item.content);p.setReportUpdatedAt(item.createdAt);setView("preview")}}><span className="history-index">{String(p.reportHistory.length-index).padStart(2,"0")}</span><div><strong>{item.title}</strong><small>{item.createdAt}</small></div><ChevronRight size={15}/></button>)}</div></aside></div>:<section className="panel"><Empty text="还没有研究报告"/><div className="empty-note">点击「生成报告」，整理当前项目材料并创建第一条生成记录。</div></section>}
 </>;
}
